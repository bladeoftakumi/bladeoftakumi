...
const NAV_TABS = [
  ...
  { key: "organizer", label: "Outreach Organizer", Icon: IconClipboard },
  { key: "library", label: "Open Itineraries", Icon: IconBookmark },
...
function HomeDashboard({ user, lib, onNavigate, onPlanUnvisited }) {
  ...
  const org = useAMemo(() => {
    try {
      const d = JSON.parse(localStorage.getItem("rihla_organizer_v1"));
      if (!d || !d.records || !d.records.length) return null;
      let contacted = 0, booked = 0, confirmed = 0;
      d.records.forEach((r) => {
        const s = typeof statusOf === "function" ? statusOf(r.status) : r.status;
        if (s && s !== "Not Started") contacted++;
        if (s === "Meeting Booked") booked++;
        if (s === "Confirmed") confirmed++;
      });
      return { city: d.meta && d.meta.city, schools: d.records.length, contacted, booked, confirmed };
    } catch (e) { return null; }
  }, []);
    ...
    <div className="page page-wide home">
      ...
      {org && (
        <div className="home-org">
          <div className="home-section-label">Outreach Organizer{org.city ? " \u00b7 " + org.city : ""}</div>
          <div className="org-stats">
            <StatPill num={org.schools} label="Schools" />
            <StatPill num={org.contacted} label="Contacted" />
            <StatPill num={org.booked} label="Meetings booked" />
            <StatPill num={org.confirmed} label="Confirmed" accent />
          </div>
        </div>
      )}
      ...
      <div className="choice-grid home-choice">
        ...
        <button className="choice-card" onClick={() => onNavigate("directory")}>
          ...
          <span className="choice-desc">Every place, contact and visit in one searchable record — your source of truth.</span>
          <span className="choice-cta">Open directory <IconArrowRight size={15} /></span>
        </button>
        <button className="choice-card" onClick={() => onNavigate("organizer")}>
          <span className="choice-icon"><IconClipboard size={24} /></span>
          <span className="choice-name">Outreach Organizer</span>
          <span className="choice-desc">Log contacts, book appointments and track each school&rsquo;s status.</span>
        ...
        <button className="choice-card" onClick={() => onNavigate("library")}>
          ...
          <span className="choice-name">Open Itineraries</span>
    ...
    <div className="page landing">
      ...
      <div className="choice-grid">
        ...
        <button className="choice-card" onClick={onOrganizer}>
          <span className="choice-icon"><IconClipboard size={24} /></span>
          <span className="choice-name">Outreach Organizer</span>
          <span className="choice-desc">Upload your planner export to log contacts, book appointments and track each school&rsquo;s status.</span>
          <span className="choice-cta">Open organizer <IconArrowRight size={15} /></span>
        </button>
      </div>
